...
async def test_login_success(client_no_auth):
    await client_no_auth.post("/api/auth/register", json={"username": "charlie", "password": "secret123"})
    resp = await client_no_auth.post(
...
async def test_login_wrong_password(client_no_auth):
    await client_no_auth.post("/api/auth/register", json={"username": "dave", "password": "secret123"})
    resp = await client_no_auth.post(
...
async def test_login_disabled_user(client_no_auth, db_session):
    ...
    user = await svc.create_user(username="eve", password="secret123")
...
async def test_me_normal(client_no_auth, make_user):
    ...
    user = await make_user("frank", "secret123", is_superuser=True)
